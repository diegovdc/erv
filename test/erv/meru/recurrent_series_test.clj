(ns erv.meru.recurrent-series-test
  (:require
   [clojure.test :refer [deftest is testing]]
   [erv.math.pascals-triangle :as pascals-triangle]
   [erv.meru.recurrent-series :as subject]
   [erv.types :refer [MeruRecurrentSeriesData]]
   [malli.core :as m]))

(deftest recurrent-series-test
  (testing "The Malli type is up to date"
    (let [data (subject/recurrent-series
                {:seed [1 1 1]
                 :formula :meta-slendro})]
      (is (m/validate MeruRecurrentSeriesData data)
          (m/explain MeruRecurrentSeriesData data))))
  (testing "Basic usage"
    (let [meta-slendro-series {:convergence-double 1.324717957244746,
                               :convergence 53406819691/40315615410,
                               :convergence-index 84,
      ...
      (testing "Can use a `:seed`  and a `:formula`"
        (is (= meta-slendro-series
               (-> (subject/recurrent-series {:seed [1 1 1]
                                              :formula :meta-slendro})
                   (select-keys [:convergence-double :convergence :convergence-index :series])))))
